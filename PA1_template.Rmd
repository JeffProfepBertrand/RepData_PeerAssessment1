...
Before loading and pre-processing the data, the global environment will be cleaned and libraries to be used in this assessment will be loaded to memory.  
...
Once those libraries are loaded into memory, the next step of the project is to load the data from the internet into a data frame.  If proxy server restrictions does not allow RStudio to open data from the internet (which is the case for me due to IT protocols), download data in a folder on computer and read from there.  
...
The first step is to transform the data into a usable format (e.g. including the creation of a POSIXct variable), with the addition of creating new variables that will be useful in future step of the project. This includes creating a variable representing the day of the week (will be used for imputation) as well as a dichotomous variable for Week days/Weekend days that will also be used later.  Those variable are created upfront, but could have been created later in the analysis.
...
Before calculating the mean and the median for the total number of steps taken per day, we need to calculate the total number of steps taken daily.  The first part of the analysis will disregard missing values.

```{r total_daily_step, echo=TRUE}
...
Figure 1, below, is a histogram of the total number of steps taken each day (ignoring missing).
...
As indicated on Figure 1, the mean number of steps taken per day over the period analysed is `r format(round(mean_daily_step$mean), big.mark=" ", scientific=FALSE, trim=TRUE)` while the median number of steps taken is `r format(round(median_daily_step$median), big.mark=" ", scientific=FALSE, trim=TRUE)`.

## Mean number of steps taken per 5 minutes intervals (ignoring missing)

We will now calculate the mean number of steps taken per 5 minutes intervals, once again ignoring missing.  In order to make it easier to plot the time period, we will create a variable that follow the grouping position (i.e.: hour and minute) to make it easier to plot results nicely over the time period.  We also identify which daily 5-minute interval contain, on average, the maximum number of steps taken.

```{r mean_interval_step, echo=TRUE}
...
Figure 2, below, is a line graph of the mean number of steps taken per 5 minutes intervals (ignoring missing).
...
As indicated on Figure 2, the maximum average number steps taken per period of 5 minutes is  `r round(max_interval_step$mean)` and it takes place at `r format(max_interval_step$time, format="%b %d %Y")`.

## Impact of missing values (and imputation of those)

The first step taken here is to calculate and report the total number of missing values in the data.  We have a more detailled look at the observation to ensure which variable(s) has missings.
...
In order to impute missing, we will look at daily patterns (e.g.: days of the week) by hours and 5 minutes intervals using the variables created earlier.  We will then create a new data set where we will replace missing observations by calculated means.  Before moving forward, we ensure we don't have anymore missing observations.  
...
Once this is done, we used the same approach as before to calculate the mean and the median for the total number of steps taken per day, but this time with imputed values replacing missing ones.

```{r total_daily_step_imputed, echo=TRUE}
...
Figure 3, below, is a histogram of the total number of steps taken each day with missing replaced by imputed values.
...
As indicated on Figure 3, the mean number of steps taken per day over the period analysed is `r format(round(mean_daily_step_imputed$mean), big.mark=" ", scientific=FALSE, trim=TRUE)` while the median number of steps taken is `r format(round(median_daily_step_imputed$median), big.mark=" ", scientific=FALSE, trim=TRUE)`.

As we can see, not only does the shape of Figure 3 changes compare to figure 1, but the mean number of steps taken per day over the period analysed increase by `r format(round(mean_daily_step_imputed$mean) - round(mean_daily_step$mean), big.mark=" ", scientific=FALSE, trim=TRUE)` while the median increase by `r format(round(median_daily_step_imputed$median) - round(median_daily_step$median), big.mark=" ", scientific=FALSE, trim=TRUE)`.

## Differences in activity patterns between weekdays and weekends

The last step of this project is to assess if there are differences in activity patterns between weekdays and weekends (using the data with the filled-in missing values).  First, let's group the data using the weekday variable prepared in the first steps of this project.  The next step is do do similar calculations as done for Figure 2. 

```{r mean_interval_step_imputed, echo=TRUE}
...
Figure 4, below, is a line graph of with the mean number of steps taken per 5 minutes intervals (with missing replaced by imputed values) for week days and weekend days.
...
As we clearly see from figure 4, the daily patterns appears to be different for week days and weekend days. The maximum average number steps taken per period of 5 minutes, during week days, is `r round(max_interval_step_imputed$mean[1])` and it takes place at `r format(max_interval_step_imputed$time[1], format="%b %d %Y")`. This compares respectively to `r round(max_interval_step_imputed$mean[2])` steps taking place at `r format(max_interval_step_imputed$time[2], format="%b %d %Y")` for weekend days.
